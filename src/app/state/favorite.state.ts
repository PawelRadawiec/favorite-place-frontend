...
import {
  defaultFavoriteStateModel,
  FavoriteStateModel,
} from './favorite-state.model';
...
export class FavoriteState {
  ...
  create(
    context: StateContext<FavoriteStateModel>,
    action: FavoriteActions.Create
  ) {
    return this.favoriteService
      .create(action.favorite)
      .pipe(mergeMap(() => this.store.dispatch(new FavoriteActions.GetAll())));
  }
  ...
  getAll(context: StateContext<FavoriteStateModel>) {
    return this.favoriteService
      ...
      .pipe(
        mergeMap((response) =>
          this.store.dispatch(new FavoriteActions.FavoritesLoaded(response))
        )
      );
  }
